...
import ru.netology.transfer.page.VerifcationPage;
import ru.netology.transfer.page.TransferPage;

public class TepmplateSteps {
    private static LoginPage loginPage;
    private static DataUser dataUser;
    private static DashboardPage dashboardPage;
    private static VerifcationPage verifcationPage;
    private static TransferPage transferPage;

    @Пусть("пользователь залогинен с именем {string} и паролем {string}")
    public void verifcation(String login, String password, String verification) {
        loginPage = Selenide.open("http://localhost:9999", LoginPage.class);
        verifcationPage = loginPage.validLogin(new DataUser.UserUnfo(login, password, verification));
        dashboardPage = verifcationPage.validVerify(verification);
